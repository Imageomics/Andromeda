// https://observablehq.com/@hanliuxxx/andromeda_js/2@1998
import define1 from "./a33468b95d0b15b0@808.js";
import define2 from "./b2bbebd2f186ed03@1786.js";

function _1(md){return(
md`# Andromeda_JS`
)}

function _relevance(rangeSlider,themes){return(
rangeSlider({
  min: 0,
  max: 1,
...
function _3(legend,cScale){return(
legend({
  color: cScale,
  title: "Relevance"
})
...
function _w(html,data,weights,d3)
{
  var form = html`<form>
    ${data.attrs.map(attr => 
      html`<div><input type=range name='${attr}' id='${attr}' min=0 max=2.0 step=0.01 value=0.8>${attr}</div>`)}
    </form>`;
  // nonlinear slider to weight mappings:
  function weightOfSlider(s){ return s < 1 ? s : (s-1)*(data.attrs.length-1)+1; }
  function sliderOfWeight(w){ return w < 1 ? w : (w-1)/(data.attrs.length-1)+1; }
  ...
  // slider interaction:
  data.attrs.forEach(attr => form[attr].oninput = () => {
    ...
    if (prev == 0)
      data.attrs.forEach(a => {
        if (a != attr) form[a].value = sliderOfWeight(weights[a] = remaining / (data.attrs.length-1)) });
    else
      data.attrs.forEach(a => {
        if (a != attr) form[a].value = sliderOfWeight(weights[a] = weights[a] * remaining / prev) });
    form.value = weights;
  } );
  return form;
}
...
function _scatter_chart(d3,width,height,graph,relevance_inPercent,relevance,simulation,drag,$0,cScale,invalidation)
{
  ...
  const labels = svg.append("g")
      .attr('class', 'labels')
    .selectAll("text").data(graph.vertices).join("text")
  		.text(d => d.id)
      .attr("dx", 8).attr("dy", ".35em")
      .attr('id', (d, i) => {return "text_" + i})
      .style('display', (d,i) => {
          if(relevance_inPercent[i] < relevance[0] * 100 || relevance_inPercent[i] > relevance[1] * 100){
            simulation.restart();
            return "none";
          }else{
            simulation.restart();
  ...
  const nodes = svg.append("g")
      .attr('class', 'nodes')
    .selectAll("circle").data(graph.vertices).join("circle")
      .attr("r", 7)
      ...
      .on("click", (event, d) => {
          ...
          if(d3.select(event.target).style("fill") == "blue"){
            d3.select(event.target).style('fill', (d, i) => cScale(relevance_inPercent[d.index]));
            d3.select(lineID).style('stroke', (d, i) => cScale(relevance_inPercent[d.index]));
...
function _reset_button(Inputs){return(
Inputs.button("Reset Plot")
)}

function _copy2sliders(Inputs){return(
Inputs.button([
  ["copy to Sliders", value =>value += 1]
], {value: 0})
)}

function _10(copy2sliders){return(
copy2sliders
)}

function _inversed_chart(BarChart,labeled_weights,d3,width){return(
BarChart(labeled_weights, {
  x: d => d.Weight,
  y: d => d.Name,
  yDomain: d3.groupSort(labeled_weights, ([d]) => -d.Weight, d => d.Name), // sort by descending frequency
  xFormat: "%",
  xLabel: "Frequency →",
  width,
  color: "steelblue"
...
function _12(labeled_weights){return(
labeled_weights
)}
...
function _14(weights){return(
weights
)}

function _15(copy2sliders){return(
copy2sliders
)}

function _16(selected_nodes){return(
selected_nodes
)}

function _17(updated_weights){return(
updated_weights
)}

function _18(weights){return(
weights
)}
...
function _copy_to_sliders(copy2sliders,selected_nodes,counter,attrs,updated_weights,$0,$1)
{
  copy2sliders;
  var result = {}
...
md `## Data Preprocessing for Relevance Data`
)}

function _weights(data){return(
Object.fromEntries(data.attrs.map(a => [a, 1.0]))
)}
...
function _attrs(new_data){return(
Object.keys(new_data[0])
)}
...
function _new_data(data){return(
data.map(r => {
...
function _normalized_data(attrs,new_data,d3)
{
  var stdMean = []
  for(var i = 0; i < attrs.length; i++){
    var obj = {};
...
md `## Color-Coded Scatter Plot Initialization`
)}

function _32(md){return(
md`1. Build Color Scale for the dots
...
4. Enable the drag functionality`
)}

function _cScale(d3){return(
d3.scaleSqrt([0, 50, 100], ["yellow", "red", "darkred"])
)}
...
function _34(data,w,graph,Distance,simulation,scale)
{
  ...
  simulation.force("link")
        .distance(e => e.mydistance * scale * 0.8);
  simulation.alpha(1).restart();
}


function _simulation(d3,graph,width,height){return(
d3.forceSimulation(graph.vertices)
...
function _Distance(d3,data){return(
...
function _41(width){return(
width
)}

function _scale_x(d3,width){return(
d3.scaleLinear()
  .domain([0, width]) // unit: km
  .range([-30, 30])
)}

function _scale_y(d3,height){return(
d3.scaleLinear()
...
function _47(selected_data){return(
selected_data
)}
...
function _50(FileAttachment){return(
...
md `## Distance Functions`
)}

function _euclidean_distance(){return(
...
function _manhattan_distance(){return(
...
function _pairwise_distance(selected_cords,euclidean_distance,manhattan_distance){return(
...
function _dist2D(pairwise_distance,data2Dnew){return(
pairwise_distance(data2Dnew, "euclidean")
)}

function _apply_weights(){return(
...
function _inversMDS(apply_weights,pairwise_distance,stress,dist2D,new_proposal){return(
...
function _84(FileAttachment){return(
FileAttachment("Animal_Data_Andromeda@1.csv").csv()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["Animal_Data_Andromeda@1.csv", {url: new URL("./files/45a9ebcc28d86912f7044e2a2632374ff4d9df471c2d885f3abb4edcda7e3fc654a3367ec78ac8bcd7ebb6d523476e4b11d077cbaeaa93af45159a4bbe85b7af.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("viewof relevance")).define("viewof relevance", ["rangeSlider","themes"], _relevance);
  main.variable(observer("relevance")).define("relevance", ["Generators", "viewof relevance"], (G, _) => G.input(_));
  main.variable(observer()).define(["legend","cScale"], _3);
  main.variable(observer()).define(["md"], _4);
  main.variable(observer("viewof w")).define("viewof w", ["html","data","weights","d3"], _w);
  main.variable(observer("w")).define("w", ["Generators", "viewof w"], (G, _) => G.input(_));
  main.variable(observer("viewof scale")).define("viewof scale", ["html"], _scale);
  main.variable(observer("scale")).define("scale", ["Generators", "viewof scale"], (G, _) => G.input(_));
  main.variable(observer("scatter_chart")).define("scatter_chart", ["d3","width","height","graph","relevance_inPercent","relevance","simulation","drag","mutable selected_nodes","cScale","invalidation"], _scatter_chart);
  main.variable(observer("viewof reset_button")).define("viewof reset_button", ["Inputs"], _reset_button);
  main.variable(observer("reset_button")).define("reset_button", ["Generators", "viewof reset_button"], (G, _) => G.input(_));
  main.variable(observer("viewof copy2sliders")).define("viewof copy2sliders", ["Inputs"], _copy2sliders);
  main.variable(observer("copy2sliders")).define("copy2sliders", ["Generators", "viewof copy2sliders"], (G, _) => G.input(_));
  main.variable(observer()).define(["copy2sliders"], _10);
  main.variable(observer("inversed_chart")).define("inversed_chart", ["BarChart","labeled_weights","d3","width"], _inversed_chart);
  ...
  main.variable(observer("copy_to_sliders")).define("copy_to_sliders", ["copy2sliders","selected_nodes","counter","attrs","updated_weights","mutable counter","mutable weights"], _copy_to_sliders);
  main.variable(observer()).define(["md"], _21);
  main.define("initial weights", ["data"], _weights);
  main.variable(observer("mutable weights")).define("mutable weights", ["Mutable", "initial weights"], (M, _) => new M(_));
  main.variable(observer("weights")).define("weights", ["mutable weights"], _ => _.generator);
  main.variable(observer("weight_list")).define("weight_list", ["attrs","w"], _weight_list);
  ...
  main.variable(observer("normalized_data")).define("normalized_data", ["attrs","new_data","d3"], _normalized_data);
  main.variable(observer("applied_weight_data")).define("applied_weight_data", ["normalized_data","attrs","weight_list"], _applied_weight_data);
  main.variable(observer("relv_value")).define("relv_value", ["applied_weight_data","attrs"], _relv_value);
  main.variable(observer("relevance_inPercent")).define("relevance_inPercent", ["relv_value"], _relevance_inPercent);
  main.variable(observer()).define(["md"], _31);
  main.variable(observer()).define(["md"], _32);
  main.variable(observer("cScale")).define("cScale", ["d3"], _cScale);
  main.variable(observer()).define(["data","w","graph","Distance","simulation","scale"], _34);
  main.variable(observer("simulation")).define("simulation", ["d3","graph","width","height"], _simulation);
  main.variable(observer("data")).define("data", ["reset_button","d3","FileAttachment"], _data);
  main.variable(observer("Distance")).define("Distance", ["d3","data"], _Distance);
  main.variable(observer("graph")).define("graph", ["reset_button","copy2sliders","data","Distance"], _graph);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("height")).define("height", _height);
  main.variable(observer()).define(["width"], _41);
  main.variable(observer("scale_x")).define("scale_x", ["d3","width"], _scale_x);
  main.variable(observer("scale_y")).define("scale_y", ["d3","height"], _scale_y);
  main.variable(observer()).define(["md"], _44);
  main.variable(observer()).define(["md"], _45);
  main.define("initial selected_nodes", ["reset_button"], _selected_nodes);
  main.variable(observer("mutable selected_nodes")).define("mutable selected_nodes", ["Mutable", "initial selected_nodes"], (M, _) => new M(_));
  main.variable(observer("selected_nodes")).define("selected_nodes", ["mutable selected_nodes"], _ => _.generator);
  main.variable(observer()).define(["selected_data"], _47);
  main.variable(observer("selected_data")).define("selected_data", ["selected_nodes","data"], _selected_data);
  main.variable(observer("selected_labels")).define("selected_labels", ["selected_data"], _selected_labels);
  main.variable(observer()).define(["FileAttachment"], _50);
  main.variable(observer("selected_cords")).define("selected_cords", ["selected_nodes","scale_x","scale_y"], _selected_cords);
  main.variable(observer()).define(["md"], _52);
  main.variable(observer("euclidean_distance")).define("euclidean_distance", _euclidean_distance);
  main.variable(observer("manhattan_distance")).define("manhattan_distance", _manhattan_distance);
  main.variable(observer("pairwise_distance")).define("pairwise_distance", ["selected_cords","euclidean_distance","manhattan_distance"], _pairwise_distance);
  main.variable(observer()).define(["md"], _60);
  main.variable(observer("dataHDpart")).define("dataHDpart", ["selected_data"], _dataHDpart);
  main.variable(observer("data2Dnew")).define("data2Dnew", ["selected_cords"], _data2Dnew);
  ...
  main.variable(observer("dist2D")).define("dist2D", ["pairwise_distance","data2Dnew"], _dist2D);
  main.variable(observer("apply_weights")).define("apply_weights", _apply_weights);
  main.variable(observer("inversMDS")).define("inversMDS", ["apply_weights","pairwise_distance","stress","dist2D","new_proposal"], _inversMDS);
  main.variable(observer("updated_weights")).define("updated_weights", ["selected_cords","inversMDS","dataHDpart","data2Dnew"], _updated_weights);
  main.variable(observer("labeled_weights")).define("labeled_weights", ["attrs","updated_weights"], _labeled_weights);
